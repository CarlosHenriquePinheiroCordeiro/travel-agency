...
export class MailService {
    ...
    constructor(@Inject('MAIL_SERVICE') private rabbitClient: ClientProxy, private configService: ConfigService) {}
    ...
    private ticketToMailMsg(createTicketDto: CreateTicketDto): MailDto {
        ...
        Object.assign(mail, { senderEmail: this.configService.get<string>('SENDER_EMAIL') });
        Object.assign(mail, { senderPass: this.configService.get<string>('SENDER_EMAIL_PASS') });
        return mail;
    }

    private getTemplateStringFlightMail(createTicketDto: CreateTicketDto) {
        return `${createTicketDto.owner} - TRIP ${createTicketDto.tripId}`;
    }
